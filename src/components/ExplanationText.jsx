...
const ExplanationText = ({operatorType, explanationCase}) => {
  ...
  const renderText = () => {
    if(operatorType === "Start") return <div className={styles.text}>Start step by step visualization</div>
    ...
    if(operatorType === "Right parenthesis") return <div className={styles.text}>If character is a right parenthesis if <p className={`${styles.statement} ${explanationCase.stringHasValue ? styles.true : styles.unmached}`}>string number contains any number </p>push this number to the output array then while <p className={`${styles.statement} ${trueStatements.includes(0) ? styles.true : styles.false}`}>character at the top of the stack is not "("</p> and <p className={`${styles.statement} ${trueStatements.includes(0) ? styles.true : styles.false}`}>stack is not empty</p> push operator at the top of the stack to the output array then delete the left parenthesis</div>
    if(operatorType === "Outputing") return <div className={styles.text}>If <p className={`${styles.statement} ${explanationCase.stringHasValue ? styles.true : styles.unmached}`}>string number representatnion contains any number left</p> or <p className={`${styles.statement} ${explanationCase.stackHasValue ? styles.true : styles.unmached}`}>the stack isn't empty</p> push everything to the output</div>
