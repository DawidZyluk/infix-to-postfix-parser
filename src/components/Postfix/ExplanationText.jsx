...
const ExplanationText = ({operatorType, explanationCase}) => {
  ...
  const renderText = () => {
    ...
    if(operatorType === "Letter") return <div className={styles.text}><div className={styles.title}>{operatorType}</div>If character is a letter append it to the function strign</div>
    if(operatorType === "Negation") return <div className={styles.text}><div className={styles.title}>{operatorType}</div><p className={`${styles["overall-statement"]} ${overallValue ? styles["overall-true"] : styles["overall-unmached"]}`}><span className={`${styles.statement} ${trueStatements.includes(0) ? styles.true : styles.unmached}`}>If "-" appears at the beggining of the expression </span> or <span className={`${styles.statement} ${trueStatements.includes(1) ? styles.true : styles.unmached}`}>"(" is at the top of the operator stack and the incoming character is "-"</span> or <span className={`${styles.statement} ${trueStatements.includes(2) ? styles.true : styles.unmached}`}>incoming char is "-" and any other operator is at the top of the operator stack</span></p> <p className={`${styles["overall-statement"]} ${styles["overall-true"]}`}>then <span className={`${styles.statement} ${explanationCase.isNegative.value ? styles.true : styles.unmached}`}>if number isn't already negated prepend "-" to the string number representatnion</span> or <span className={`${styles.statement} ${!explanationCase.isNegative.value ? styles.true : styles.unmached}`}>if string number is already negated delete the negation</span></p></div>
    ...
    if(operatorType === "Left parenthesis") return <div className={styles.text}><div className={styles.title}>{operatorType}</div>If character is a left parenthesis and <p className={`${styles.statement} ${explanationCase ? styles.true : styles.unmached}`}>function string contains a valid function name</p> then push both to the operator stack else just push the parenthesis</div>
    if(operatorType === "Right parenthesis") return <div className={styles.text}><div className={styles.title}>{operatorType}</div><p className={`${styles["overall-statement"]} ${explanationCase.stringHasValue ? styles["overall-true"] : styles["overall-unmached"]}`}><span className={`${styles.statement} ${explanationCase.stringHasValue ? styles.true : styles.unmached}`}>If string number contains any number </span>push this number to the output array</p> <p className={`${styles["overall-statement"]} ${overallValue ? styles["overall-true"] : styles["overall-false"]}`}>then while <span className={`${styles.statement} ${trueStatements.includes(0) ? styles.true : styles.false}`}>character at the top of the stack is not "("</span> and <span className={`${styles.statement} ${trueStatements.includes(0) ? styles.true : styles.false}`}>stack is not empty</span> push operator at the top of the stack to the output array then delete the left parenthesis</p></div>
