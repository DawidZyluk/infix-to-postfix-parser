...
const ExplanationText = ({operatorType, explanationCase}) => {
  ...
  const renderText = () => {
    ...
    if(operatorType === "Negation") return <div className={styles.text}><div className={styles.title}>{operatorType}</div>If <p className={`${styles["overall-statement"]} ${overallValue ? styles["overall-true"] : styles["overall-unmached"]}`}><span className={`${styles.statement} ${trueStatements.includes(0) ? styles.true : styles.unmached}`}>"-" appears at the beggining of the expression </span> or <span className={`${styles.statement} ${trueStatements.includes(1) ? styles.true : styles.unmached}`}>"(" is at the top of the operator stack and the incoming character is "-"</span> or <span className={`${styles.statement} ${trueStatements.includes(2) ? styles.true : styles.unmached}`}>incoming char is not "(" and any other operator is at the top of the operator stack</span></p> then prepend "-" to the string number representatnion</div>
