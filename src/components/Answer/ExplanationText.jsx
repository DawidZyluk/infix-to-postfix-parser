...
const ExplanationText = ({operatorType, data, index, maxIndex}) => {
  ...
  // data[index-1][data[index-1].length-2] FIX THIS !!!
  ...
  const renderText = () => {
    ...
    if(precedence.has(operatorType)) return <div className={styles.text}><div className={styles.title}>Operator</div>If character is an operator take two last elements ({data[index-1][data[index-1].length-2]} and {data[index-1][data[index-1].length-1]}) of the stack. Execute current operation ({data[index-1][data[index-1].length-2]} {operatorType} {data[index-1][data[index-1].length-1]}). Then put the result ({data[index][data[index].length-1]}) on top of the stack</div>
